...
public:
	...
	void editExamsResults(StudentNode* sn) {
		...
		sesMenu->addItem("Сессия 1");
		sesMenu->addItem("Сессия 2");
		sesMenu->addItem("Сессия 3");
		sesMenu->addItem("Сессия 4");
		sesMenu->addItem("Сессия 5");
		sesMenu->addItem("Сессия 6");
		sesMenu->addItem("Сессия 7");
		sesMenu->addItem("Сессия 8");
		sesMenu->addItem("Сессия 9");
		while (result != exit) {
			sesMenu->eraseTitle();
			sesMenu->addTitleItem("Просмотр/изменение/добавление данных о оценках за все сессии");
			sesMenu->addTitleItem("Выберите сессию для просмотра информации об оценках");
			sesMenu->run();
